...
select op.payment_value,p.product_id,oi.order_id from order_payments op
join order_items oi on op.order_id=oi.order_id
join products p on oi.product_id=p.product_id ;

select p.product_id,pcn.product_category_name_english from products p
join product_category_name pcn on p.product_id=pcn.product_category_name
