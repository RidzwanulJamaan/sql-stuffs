...
(select date_format (review_answer_timestamp,'%Y-%d') as review_date ,sum(case when ow.review_score =5 then 1 else 0 end) as score_5_count,count(distinct o.orders_id)as total_order from orders  o
...
select date_format (review_answer_timestamp,'%Y-%d') as review_date from order_reviews;
